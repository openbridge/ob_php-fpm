...
if [[ $(find ${APP_DOCROOT} ! -perm 755 -type d | wc -l) -gt 0 ]] || [[ $(find ${APP_DOCROOT} ! -perm 644 -type f | wc -l) -gt 0 ]]; then
...
if [[ $(find ${APP_DOCROOT} ! -user www-data | wc -l) -gt 0 ]]; then
    ...
    echo "OK: www-data (user:group) ownership looks corect on ${APP_DOCROOT}"
fi
